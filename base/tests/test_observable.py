...
class TestObservable:
    ...
    def test_expected_value_for_standard_projection(self, standard_projections, valid_qubit):
        """[正常系]: 標準基底に対する射影観測の期待値"""
        target_qubit = Qubit(valid_qubit[0], valid_qubit[1])
        for index in [0 ,1]:
            assert standard_projections[index].expected_value(target_qubit) == abs(target_qubit.amplitudes[index]) ** 2
