...
class TestQubits:
    ...
    def test_valid_qubit_by_list(self, valid_orthogonal_non_degrated_qubit_list):
        ...
        qubit = Qubits(
            valid_orthogonal_non_degrated_qubit_list["probabilities"], valid_orthogonal_non_degrated_qubit_list["pure_qubits"]
        )
        assert np.all(
            np.round(
                np.array(qubit.eigen_values) - np.array(valid_orthogonal_non_degrated_qubit_list["probabilities"]),
                APPROX_DIGIT
            ) == 0.0
        )
        for index in range(len(qubit.eigen_values)):
            assert np.all(
                np.round(
                    ...
                    np.array(valid_orthogonal_non_degrated_qubit_list["pure_qubits"][index].amplitudes),
                    APPROX_DIGIT
                ) == 0.0
            )
